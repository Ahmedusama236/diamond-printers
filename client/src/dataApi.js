...
async function selectRows(table, options = {}) {
  let query = requireSupabase().from(table).select(options.columns || "*");
...
async function updateRows(table, payload, eq = []) {
  let query = requireSupabase().from(table).update(payload).select();
...
async function deleteRows(table, eq = []) {
  let query = requireSupabase().from(table).delete().select();
  for (const [column, value] of eq) {
    query = query.eq(column, value);
  }
  const { data, error } = await query;
